#' ---
#' title: "ANCOVA test for `pos.score`~`pre.score`+`scenario`"
...
#' 
## ---- echo=FALSE-------------------------------------------------------------------------------------------------------------------------------
car::Boxplot(`pos.score` ~ `scenario`, data = dat[["pos.score"]], id = list(n = Inf))
...
## ---- echo=FALSE-------------------------------------------------------------------------------------------------------------------------------
for (dv in dvs) {
  car::Boxplot(`dv` ~ `scenario`, data = sdat[[dv]] %>% cbind(dv=sdat[[dv]][[dv]]), id = list(n = Inf))  
}
...
#' ### Textual Report
#' 
#' After controlling the linearity of covariance "pre.score", ANCOVA tests with independent between-subjects variables "scenario" (gamified, non.gamified) were performed to determine statistically significant difference on the dependent varibles "pos.score". For the dependent variable "pos.score", there was statistically significant effects in the factor "pre.score" with F(1,25)=36.385, p<0.001 and ges=0.593 (effect size) and in the factor "scenario" with F(1,25)=54.279, p<0.001 and ges=0.685 (effect size).
#' 
#' 
#' 
#' Pairwise comparisons using the Estimated Marginal Means (EMMs) were computed to find statistically significant diferences among the groups defined by the independent variables, and with the p-values ajusted by the method "bonferroni". For the dependent variable "pos.score", the mean in the scenario="gamified" (adj M=8.668 and SD=0.893) was significantly different than the mean in the scenario="non.gamified" (adj M=6.974 and SD=0.949) with p-adj<0.001.
#' 
#' 
#' 
#' ## Tips and References
